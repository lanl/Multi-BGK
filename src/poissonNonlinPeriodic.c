...
void electronSource(gsl_vector *phi, double *g, double *gPrime, double ne0, double *Te);
void electronSource_TF(gsl_vector *phi, double *g, double *gPrime, double mu, double *Te);
...
void PoissNonlinPeriodic1D(int N, double *source, double dx, double Lx, double *phi, double *Te) 
{
  ...
  while ( (( relErr > relTol ) || ( absErr > absTol )) && (loop < 50) ) {
    ...
    for(i=0;i<N;i++) {
      ...
      gsl_vector_set(RHS,i, -gsl_vector_get(phixx,i) + 4.0*M_PI*E_02_CGS*(source[i] - g[i]));
    }
  ...
  */
  ...
  while ( (( relErr > relTol ) || ( absErr > absTol )) && (loop < 50) ) {
    ...
    for(i=0;i<N;i++) {
      gsl_matrix_set(B,i,i,gsl_matrix_get(A,i,i) + 4.0*M_PI*E_02_CGS*gPrime[i]);
      gsl_vector_set(RHS,i, 4.0*M_PI*E_02_CGS*(source[i] - g[i] + gPrime[i]*gsl_vector_get(phiVec,i)));
    }
...
void PoissLinPeriodic1D(int N, double *source, double dx, double Lx, double *phi, double *Te) 
{
  ...
  //initialize the matrix A
  gsl_matrix_set(A,0,0  , 2.0/dx/dx + ne0_diag/Te[i]);
  gsl_matrix_set(A,0,1  ,-1.0/dx/dx);
  gsl_matrix_set(A,0,N-1,-1.0/dx/dx);  
  for(i=1;i<(N-1);i++) {
  ...
  gsl_matrix_set(A,N-1,N-1, 2.0/dx/dx + ne0_diag/Te[i]);
  gsl_matrix_set(A,N-1,N-2,-1.0/dx/dx);
  gsl_matrix_set(A,N-1,0  ,-1.0/dx/dx);
...
-(e phi)_xx + [ ( 8pi e^2 (2pim_e)^3/2 Te^1/2 / (2pi hbar)^3) F_(-1/2)(beta mu) ] (e phi) 
eV / cm^2     [       eV-cm   g        eV            eV-s              1/eV eV  ]  eV           
...
void PoissLinPeriodic1D_TF(int N, double *source, double dx, double Lx, double *phi, double *Te) 
{
  ...
  double diag = 8.0*M_PI*E_02_CGS*pow(2.0*M_PI*M_ELEC_CGS*ERG_TO_EV_CGS,1.5)/pow(2.0*M_PI*HBAR_CGS,3);
  //double diag = F_mhalf*8.0*M_PI*E_02_CGS*pow(2.0*M_PI*M_ELEC_CGS*Te*ERG_TO_EV_CGS,1.5)/pow(2.0*M_PI*HBAR_CGS,3)/Te;
  //                             eV cm                g^3/2       eV^3/2 s^3 / g^3/2 cm^3         eV^3 s^3
 
  double fdfac = sqrt(M_PI)*0.5;
  
  //initialize the matrix A
  F_mhalf = fdfac*gsl_sf_fermi_dirac_mhalf(mu/Te[0]);
  gsl_matrix_set(A,0,0  , 2.0/dx/dx + diag*sqrt(Te[0])*F_mhalf );   //1/cm^2
  gsl_matrix_set(A,0,1  ,-1.0/dx/dx);
  gsl_matrix_set(A,0,N-1,-1.0/dx/dx);  
  for(i=1;i<(N-1);i++) {
    F_mhalf = fdfac*gsl_sf_fermi_dirac_mhalf(mu/Te[i]);
    gsl_matrix_set(A,i,i  , 2.0/dx/dx + diag*sqrt(Te[i])*F_mhalf ); //1/cm^2
    gsl_matrix_set(A,i,i-1,-1.0/dx/dx);
    gsl_matrix_set(A,i,i+1,-1.0/dx/dx);
  }
  F_mhalf = fdfac*gsl_sf_fermi_dirac_mhalf(mu/Te[N-1]);
  gsl_matrix_set(A,N-1,N-1, 2.0/dx/dx + diag*sqrt(Te[N-1])*F_mhalf ); //1/cm^2
  gsl_matrix_set(A,N-1,N-2,-1.0/dx/dx);
  gsl_matrix_set(A,N-1,0  ,-1.0/dx/dx);
  ...
  for(i=0;i<N;i++) {
    ...
    gsl_vector_set(RHS,i,4.0*M_PI*E_02_CGS*(source[i] - F_half*2.0*pow(2.0*M_PI*M_ELEC_CGS*Te[i]*ERG_TO_EV_CGS,1.5)/pow(2.0*M_PI*HBAR_CGS,3)));  //eV /cm^2
  }
  //Set up the LA solve 
  gsl_permutation *Permut = gsl_permutation_alloc(N);
  int signum;
  ...
  for(i=0;i<N;i++) {
    ion_tot += source[i]*dx;
    e_tot += (F_half + gsl_vector_get(phiVec,i)*F_mhalf/Te[i])*(2.0*pow(2.0*M_PI*M_ELEC_CGS*Te[i]*ERG_TO_EV_CGS,1.5)/pow(2.0*M_PI*HBAR_CGS,3))*dx;
  }
  //printf("Charges:| ion: %g  electron %g abs diff %g reldiff %g \n",ion_tot,e_tot,fabs(ion_tot-e_tot),fabs(ion_tot-e_tot)/ion_tot);
...
-(e phi)_xx = 4pi e^2   (source - 2(2pi m_e T_e)^(3/2) / (2 pi hbar)^3 F_(1/2)(beta (mu + e phi)) )
  eV / cm^2       ev-cm  1/cm^3         g   eV                 eV-s            1/eV eV    eV
...
void PoissNonlinPeriodic1D_TF(int N, double *source, double dx, double Lx, double *phi, double *Te) 
{
  ...
  */
  ...
  for(i=0;i<N;i++) {
    ion_tot += source[i]*dx;
    e_tot += fdfac*gsl_sf_fermi_dirac_half((mu + gsl_vector_get(phiVec,i))/Te[i])*(2.0*pow(2.0*M_PI*M_ELEC_CGS*Te[i]*ERG_TO_EV_CGS,1.5)/pow(2.0*M_PI*HBAR_CGS,3))*dx;
  }
  //printf("Charges:| ion: %g  electron %g abs diff %g reldiff %g \n",ion_tot,e_tot,fabs(ion_tot-e_tot),fabs(ion_tot-e_tot)/ion_tot);
...
{
  double RHS;
  double RHS_prefac = 0.5*pow(2.0*M_PI*HBAR_CGS,3)/pow(2.0*M_PI*M_ELEC_CGS*ERG_TO_EV_CGS,1.5);
  //                                   ev^3 s^3                 g^3/2 -> eV^3/2  * cm^3
  ...
  for(i=0;i<N;i++) {
    ...
    while ( ( (absErr > absTol) || (relErr > relTol)) && (loop < 50)) {
      xnp1 = xn - (fdfac*gsl_sf_fermi_dirac_half(xn) - RHS)/(fdfac*gsl_sf_fermi_dirac_mhalf(xn));
      absErr = fabs(xnp1 - xn);
...
void electronSource_TF(gsl_vector *phi, double *g, double *gPrime, double mu, double *Te) {
  ...
  double prefac = fdfac*2.0*pow(2.0*M_PI*M_ELEC_CGS*ERG_TO_EV_CGS,1.5)/pow(2.0*M_PI*HBAR_CGS,3);

  for(i=0;i<N;i++) {
    g[i]      =             prefac*pow(Te[i],1.5)*gsl_sf_fermi_dirac_half((mu + gsl_vector_get(phi,i))/Te[i]);
    gPrime[i] = (1.0/Te[i])*prefac*pow(Te[i],1.5)*gsl_sf_fermi_dirac_mhalf((mu + gsl_vector_get(phi,i))/Te[i]);
  }
}
...
//linearly ramps electron temperature from T_start at time zero to T_end at time tfinal
