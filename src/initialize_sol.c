...
void initialize_sol_inhom(double ***f, int nint, double *int_loc,
                          double *ndens_in, double *v_in, double *T_in, int Nx,
                          double *x, int nspec, int Nv, int order, double **c,
                          double *m, double **n_oned, double ***v_oned,
                          double **T_oned) {
        ...
        for (j = 0; j < Nv; j++)
          for (k = 0; k < Nv; k++) {
            ...
            if (ndens_in[inputIndex] != 0.0) {
              f[l + order][s][vIndex] =
                  ndens_in[inputIndex] *
                  pow(m[s] / (2.0 * M_PI * T_in[inputIndex] / ERG_TO_EV_CGS),
                      1.5) *
                  exp(-(0.5 * m[s] / (T_in[inputIndex] / ERG_TO_EV_CGS)) *
                      ((c[s][i] - v_in[inputIndex]) *
                           (c[s][i] - v_in[inputIndex]) +
                       (c[s][j] * c[s][j]) + (c[s][k] * c[s][k])));
            } else {
...
void initialize_sol_load_inhom_file(int rank, int *Nx_ranks, int Nx_total, int nspec, double **n_oned,
  ...
  */
  ...
  for (sp = 0; sp < nspec; sp++) {
    printf("Species %d \n", sp);
    for (l = 0; l < Nx; l++) {
      printf("%d: n: %g v: %g T: %g\n", l, n_oned[l][sp], v_oned[l][sp][0],
             T_oned[l][sp]);
    }
  }
}
void initialize_sol_inhom_file(double ***f, int Nx, int nspec, int Nv,
                               int order, double **c, double *m,
                               double **n_oned, double ***v_oned,
                               double **T_oned) {
  // accessing multid arrays
        ...
        for (j = 0; j < Nv; j++)
          for (k = 0; k < Nv; k++) {
            ...
            if (n_oned[l][s] != 0) {
              f[l + order][s][vIndex] =
                  n_oned[l][s] *
                  pow(m[s] / (2.0 * M_PI * T_oned[l][s] / ERG_TO_EV_CGS), 1.5) *
