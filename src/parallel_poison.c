...
void get_bc_Te(double **Te_arr_allranks, int *Nx, double **T0_bcs){
    (*Te_arr_allranks)[0] = (*T0_bcs)[0];
    (*Te_arr_allranks)[(*Nx) + 1] = (*T0_bcs)[1];
}
...
void get_ramp_bc_Te(double **Te_arr_allranks, int *Nx, double *T_start, 
                    double *T_end, double *t, double *tfinal)
{
    ...
    (*Te_arr_allranks)[0] = rampTemp;
    (*Te_arr_allranks)[(*Nx) + 1] = rampTemp;
}
                    ...
                    double **T0_bcs, double ***n_bcs, double ***Z_bcs, double **Te_bcs)
{
    ...
    if(*ecouple == 1){
        ...
        if(*Te_start != *Te_ref){
            get_ramp_Te(Te_arr, Nx_rank, Te_start, Te_ref, t, tfinal);
            if(*rank == 0){
                get_ramp_bc_Te(Te_arr_allranks, Nx, Te_start, Te_ref, t, tfinal);
            }
        }else{
            get_uniform_Te(Te_arr, Nx_rank, Te_ref);
            if(*rank == 0){
                //ignore bc temps.
                (*Te_arr_allranks)[0] = *Te_ref;
                (*Te_arr_allranks)[(*Nx) + 1] = *Te_ref;
            }
        }
    }else{
        //local equilibrium dynamic electrons
    ...
    if(*rank == 0){
        ...
        (*source_allranks)[0] = 0.0;
        (*source_allranks)[(*Nx) + 1] = 0.0;
        if(*ecouple != 2){
            (*source_allranks)[0] += (*Z_bcs)[0][0]*(*n_bcs)[0][0];
            (*source_allranks)[(*Nx) + 1] += (*Z_bcs)[1][0]*(*n_bcs)[1][0];
        }else{
            (*source_allranks)[0] -= (*Z_bcs)[0][0]*(*n_bcs)[0][0];
            (*source_allranks)[(*Nx) + 1] -= (*Z_bcs)[1][0]*(*n_bcs)[1][0];
        }
        for(int i = 1; i < *nspec; i++){
            (*source_allranks)[0] += (*Z_bcs)[0][i]*(*n_bcs)[0][i];
            (*source_allranks)[(*Nx) + 1] +=  (*Z_bcs)[1][i]*(*n_bcs)[1][i];
        }
    ...
    if(*rank == 0){
        ...
        if(*order == 1){
            ...
            (*PoisPot)[0] = (*PoisPot_allranks)[0];
            (*PoisPot)[1] = (*PoisPot_allranks)[0];
            if(*numRanks == 1){
                (*PoisPot)[(*Nx_rank) + 2] = (*PoisPot_allranks)[(*Nx_rank) + 1];
                (*PoisPot)[(*Nx_rank) + 3] = (*PoisPot_allranks)[(*Nx_rank) + 1];
            }else{
        ...
        if(*numRanks > 1){
            ...
            //Deal with non-periodic BC for rightmost rank

            //set local ghost cells
            if(*order == 1){
                (*source_buf)[0] = (*PoisPot_allranks)[rankOffset - 1];
                (*source_buf)[(*Nx_ranks)[(*numRanks) - 1] + 1] = (*PoisPot_allranks)[(*Nx) + 1];
            } else if(*order == 2) {
                (*source_buf)[0] = (*PoisPot_allranks)[rankOffset - 2];
                (*source_buf)[1] = (*PoisPot_allranks)[rankOffset - 1];
                (*source_buf)[(*Nx_ranks)[(*numRanks) - 1] + 2] = (*PoisPot_allranks)[(*Nx) + 1];
                (*source_buf)[(*Nx_ranks)[(*numRanks) - 1] + 3] = (*PoisPot_allranks)[(*Nx) + 1];
            }
