...
int main(int argc, char **argv) {
  ...
  //For 1D - First dimension is species, second is position (x), third is velocity
  ...
  double vmax;
  double dvmin;
  double **c;                //first dimension is species, second is 1D velo grid points. Full velo pt is e.g. (v[0][i], v[0][j], v[0][k])
  double **wts;              //first dimension is species, second is 1D velo grid points. Full wts pt is e.g. (wts[0][i], wts[0][j], wts[0][k])
  int index; 
  ...
  **********************************/  
  ...
  if(dims == 0) {    
	...
	for(j=0;j<3;j++)
	  T0 += m[i]*n_zerod[i]*(v_zerod[i][j]-v0_zerod[j])*(v_zerod[i][j]-v0_zerod[j])/(3.0*ntot);
      }
    }      
    if(ecouple == 1) {
      if (T0 < Te_ref)
    ...
    if ((restartFlag == 2) || (restartFlag == 4)) {
      load_grid_restart(Lv, &t, input_filename); //note - this assumes uniform grid for now
      for(i=0;i<nspec;i++) {
	dv = 2.0*Lv[i]/(Nv - 1.0);
	for(j=0;j<Nv;j++) {
	  c[i][j] = -Lv[i] + dv*j;
  ...
  if(dims == 0) {
	...
	for(i=0;i<Nv;i++)
	  for(j=0;j<Nv;j++)
	    for(k=0;k<Nv;k++) {
	      index = k + Nv*(j + Nv*i);
	      f_zerod[l][index] = n_zerod[l]*pow(m[l]/(2.0*M_PI*T_zerod[l]/ERG_TO_EV_CGS),1.5)*
		exp(-(0.5*m[l]/(T_zerod[l]/ERG_TO_EV_CGS))*( (c[l][i]-v_zerod[l][0])*(c[l][i]-v_zerod[l][0]) 
  ...
  while(t < tfinal) {
	...
	if(n_zerod[i] != 0.0) {
	  ...
	  for(j=0;j<3;j++) 
	    T0 += ERG_TO_EV_CGS*m[i]*n_zerod[i]*(v_zerod[i][j] - v0_zerod[j])*(v_zerod[i][j] - v0_zerod[j])/3.0;
	}
      }
      T0 = T0/ntot;    
	...
	if(RHS_min < RHS_tol) {
	  ...
	  for(i=0;i<nspec;i++)
	    for(j=0;j<nspec;j++)
	      printf("%d %d %g\n",i,j,BGK_f_minus_eq[i][j] / BGK_f_minus_eq_init[i][j]);	    
	  break;
	}
      }
	...
	if(ecouple == 2) 
	  ...
	  for(i=0;i<nspec;i++) {
	    if(n_oned[l][i] != 0.0) {
	      ...
	      for(j=0;j<3;j++) 
		T0_oned[l] += ERG_TO_EV_CGS*m[i]*n_oned[l][i]*(v_oned[l][i][j] - v0_oned[l][j])*(v_oned[l][i][j] - v0_oned[l][j])/3.0;
	    }
	  }
