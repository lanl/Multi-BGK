      ...
      if (dims == 0) {
        ...
        if (im_ex == 0) {
          ...
          */
          ...
          printf(
              "Error - please set im_ex = 0 (explicit), 1 (linear implicit), "
        ...
        ************/
        ...
        if (outcount == dataFreq) {
          if (rank == 0) {
            for (l = 0; l < Nx_rank; l++) {
              for (i = 0; i < nspec; i++) {
                T_oned[l][i] = getTemp(m[i], n_oned[l][i], v_oned[l][i],
                                       f[l + order][i], i);
        ...
        if (order == 1) {
          ...
          if (!(BGK_type < 0)) {
            ...
            if (im_ex == 0) {
              for (l = 0; l < Nx_rank; l++) {
                BGK_ex(f[l + order], f_tmp[l + order], Z_oned[l], dt,
                       Te_arr[l]);
                for (i = 0; i < nspec; i++)
                  for (j = 0; j < Nv * Nv * Nv; j++)
              ...
              printf(
                  "Error - nonlinear implicit solve not implemented for 1D\n");
              exit(1);
              /*
                BGK_im_nonlinear();
                for (i = 0; i < nspec; i++)
        ...
        if (order == 2) {
          ...
          if (rank == 0) {
            ...
            if (numRanks > 1) {
              ...
              for (rankCounter = 1; rankCounter < numRanks; rankCounter++) {
                MPI_Recv(source_buf, 2 * Nx_ranks[rankCounter], MPI_DOUBLE,
                         rankCounter, 200 + rankCounter, MPI_COMM_WORLD,
                         &status);
                for (l = 0; l < Nx_ranks[rankCounter]; l++) {
                  source_allranks[l + rankOffset] = source_buf[0 + 2 * l];
                  Te_arr_allranks[l + rankOffset] = source_buf[1 + 2 * l];
          ...
          if (rank == 0) { // Rank 0 performs the Poisson Solve
            ...
            */
            if (poissFlavor == 0) { // no E-field
              ...
              PoissNonlinPeriodic1D(Nx, source_allranks, dx, Lx,
                                    PoisPot_allranks, Te_arr_allranks);
            else if (poissFlavor == 21) // Linear Thomas-Fermi
              PoissLinPeriodic1D_TF(Nx, source_allranks, dx, Lx,
                                    PoisPot_allranks, Te_arr_allranks);
            else if (poissFlavor == 22) // Nonlinear Thomas-Fermi
              PoissNonlinPeriodic1D_TF(Nx, source_allranks, dx, Lx,
                                       PoisPot_allranks, Te_arr_allranks);
          ...
          if (rank == 0) {
            ...
            if (numRanks > 1) {
              ...
              MPI_Send(source_buf, Nx_ranks[numRanks - 1] + 2 * order,
                       MPI_DOUBLE, numRanks - 1, numRanks - 1, MPI_COMM_WORLD);
            }
          } else {
            // Get potential from rank 0
          ...
          if (!(BGK_type == -1)) {
            ...
            for (l = 0; l < Nx_rank; l++) {
              ...
              if (im_ex == 0) {
                // Step 1
                BGK_ex(f[l + order], f_conv[l + order], Z_oned[l], dt,
                       Te_arr[l]);
                for (i = 0; i < nspec; i++)
                  for (j = 0; j < Nv * Nv * Nv; j++)
                    f_tmp[l + order][i][j] =
                ...
                printf(
                    "Error - implicit solve in 1D only implemented for first "
                    "oder solve \n");
              } else if (im_ex == 2) {
                /*
                  BGK_im_nonlinear();
