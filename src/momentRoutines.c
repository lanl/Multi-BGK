...
double getDensity(double *in, int sp) {
  ...
  for (i = 0; i < N; i++)
    for (j = 0; j < N; j++)
      for (k = 0; k < N; k++)
        result +=
            wts[sp][i] * wts[sp][j] * wts[sp][k] * in[k + N * (j + N * i)];
...
double getTemp(double m, double n, double *u, double *in, int sp) {
  ...
  for (i = 0; i < N; i++)
    for (j = 0; j < N; j++)
      for (k = 0; k < N; k++) {
        result += wts[sp][i] * wts[sp][j] * wts[sp][k] *
                  in[k + N * (j + N * i)] *
                  ((c[sp][i] - u[0]) * (c[sp][i] - u[0]) +
                   (c[sp][j] - u[1]) * (c[sp][j] - u[1]) +
                   (c[sp][k] - u[2]) * (c[sp][k] - u[2]));
      }
  ...
  for (i = 0; i < N; i++)
    for (j = 0; j < N; j++)
      for (k = 0; k < N; k++) {
        double wt =
            (wts[sp][i] * wts[sp][j] + wts[sp][k]) * in[k + N * (j + N * i)];
        out[0][0] += wt * c[sp][i] * c[sp][i];
        out[0][1] += wt * c[sp][i] * c[sp][j];
        out[0][2] += wt * c[sp][i] * c[sp][k];
        out[1][0] += wt * c[sp][j] * c[sp][i];
        out[1][1] += wt * c[sp][j] * c[sp][j];
        out[1][2] += wt * c[sp][j] * c[sp][k];
        out[2][0] += wt * c[sp][k] * c[sp][i];
        out[2][1] += wt * c[sp][k] * c[sp][j];
        out[2][2] += wt * c[sp][k] * c[sp][k];
      }
  ...
  for (i = 0; i < N; i++)
    for (j = 0; j < N; j++)
      for (k = 0; k < N; k++) {
        double wt =
            (wts[sp][i] * wts[sp][j] + wts[sp][k]) * in[k + N * (j + N * i)];
	double c2 = c[sp][i] * c[sp][i] + c[sp][j] * c[sp][j] + c[sp][k] * c[sp][k];
