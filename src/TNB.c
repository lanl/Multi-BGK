...
// Added by Abdou
// This function calculates the rate of change of the distribution of species i due to its reaction with j.
...
double GetTNB_dt(double mu, double *in, double *c1, int sp, int sp2)
{
    ...
    int i, j, k;
#pragma omp parallel for reduction (+:result) private(i,j,k,g1,g2,g3,g,E_COM,Nuclear_factor_num, Nuclear_factor_dem, Nuclear_factor, cross_section,f2)
    for(i=0;i<Nv;i++)
        for(j=0;j<Nv;j++)
            for(k=0;k<Nv;k++) {

                g1 = (c1[0]-c[sp2][i]); //*wts[sp][i]*wts[sp][j]*wts[sp][k];  // v_D-v_T=relative velocity
                g2 = (c1[1]-c[sp2][j]); //*wts[sp][i]*wts[sp][j]*wts[sp][k];
                g3 = (c1[2]-c[sp2][k]); //*wts[sp][i]*wts[sp][j]*wts[sp][k];

                g  = sqrt(pow(g1,2)+pow(g2,2)+pow(g3,2)); // Calculate the norm of the relative velocity here.
//                double mu=m[i]*m2[j]/(m[i]+m2[j]);
                E_COM=0.5*mu*pow(g,2)*ERG_TO_EV_CGS*1e-3; // Energy of the center-of-mass in keV

                Nuclear_factor_num = a1  + E_COM*(a2+E_COM*(a3+E_COM*(a4+E_COM*a5)));
                Nuclear_factor_dem = 1.0 + E_COM*(b1+E_COM*(b2+E_COM*(b3+E_COM*b4)));
...
double GetReactivity_dt(double mu, double *in,double *in2, int sp, int sp2)
{
    ...
    int i, j, k, l, m, n;
#pragma omp parallel for reduction (+:result) private(i,j,k,l,m,n,g1,g2,g3,g,E_COM,Nuclear_factor_num, Nuclear_factor_dem, Nuclear_factor, cross_section,f1,f2)
    for(i=0;i<Nv;i++)
        for(j=0;j<Nv;j++)
            for(k=0;k<Nv;k++) 
	    for(l=0;l<Nv;l++)
	    ...
	    for(n=0;n<Nv;n++) {
                ...
                g1 = (c[sp][i]-c[sp2][l]); //*wts[sp][i]*wts[sp][j]*wts[sp][k];  // v_D-v_T=relative velocity
                ...
                cross_section=Nuclear_factor*exp(-B_G/sqrt(E_COM))/E_COM; // millibarn
		cross_section *= 1e-27; // converted from mbarn to cm^2
                ...
                result += g*cross_section*f1*in[k + Nv*(j + Nv*i)]*f2*in2[n + Nv*(m + Nv*l)];
                //       cm/s cm^2           /cm^3                    /cm^3       
                //     -> 1 / s cm^3
            }
    ...
    for(i=0;i<Nv;i++)
        for(j=0;j<Nv;j++)
            for(k=0;k<Nv;k++) {
                ...
                g1 = (c1[0]-0.*c[sp2][i]); //*wts[sp][i]*wts[sp][j]*wts[sp][k];  // v_D-v_T=relative velocity
                ...
                for(l=0;l<Nv;l++)
                    for(m=0;m<Nv;m++)
                        for(n=0;n<Nv;n++) {
                            ...
                            g1 = (c[sp][i]-0.*c[sp2][l]); //*wts[sp][i]*wts[sp][j]*wts[sp][k];  // v_D-v_T=relative velocity
                            g2 = (c[sp][j]-0.*c[sp2][m]); //*wts[sp][i]*wts[sp][j]*wts[sp][k];
                            g3 = (c[sp][k]-0.*c[sp2][n]); //*wts[sp][i]*wts[sp][j]*wts[sp][k];
