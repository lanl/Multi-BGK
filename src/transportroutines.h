...
//  Periodic + MPI not currently implemented lv - Semi-length of velocity domain
//  (v is assumed to be in [-lv,lv] xnodes - ordered array of locations of the
//  center of each node dxnodes - ordered array with size of each node Lx -
//  width of x domain vel - array of velocity grid points for one direction
//  (full 3d velocity is just this tensored) IC - flag that tells type of
//  problem being solved (and thus, boundary condition) - currently just set to
//  6 for periodic BCs, other options from before are removed TWall_in -
//  specifies temperature of walls for certian BCs, can be ignored otherwise
